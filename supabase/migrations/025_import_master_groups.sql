-- =============================================================================
-- 025_import_master_groups.sql
-- The import learns about master groups.
--
-- Two changes, both needed before the real data goes in:
--
--   * "master_group" becomes an importable sheet, so Parle, Current and Others
--     — and anything added later — can come from the workbook rather than SQL.
--     024 already created those three; importing them again simply reports
--     them as existing.
--
--   * The Product Groups sheet gains a REQUIRED master_code column. Every
--     product group has to say which master group it belongs to, because that
--     is how a product's master group is worked out, and that in turn is what
--     decides which bill a shop's dues sit under.
--
-- Sheet order becomes: Master Groups, Routes, Product Groups, Suppliers,
-- Parties, Products. Later sheets point at earlier ones, and the import screen
-- walks them in that order for you.
--
-- Master group codes are upper-cased on the way in, so "parle" and "Parle"
-- both find PARLE. Everything else about the import is unchanged — same
-- validation, same all-or-nothing rule, same three answers to a code that
-- already exists.
--
-- Run this file once in the Supabase SQL Editor, AFTER 024. Safe to run more
-- than once.
-- =============================================================================

-- Belt and braces. 023 already dropped the four-argument version this replaces,
-- but if these files are ever run out of order, two functions differing only by
-- a defaulted argument make EVERY import call ambiguous — and the error when
-- that happens says nothing useful about why.
...
begin
  ...
  if p_entity not in ('master_group', 'route', 'product_group',
                      'supplier', 'party', 'product') then
    raise exception 'Unknown entity "%". Expected master_group, route, product_group, supplier, party or product.',
  ...
  if p_entity = 'product_group' then
    select v_errors || coalesce(jsonb_agg(err), '[]'::jsonb) into v_errors
    from (
      select app.imp_err(row_no, 'master_code', r ->> 'master_code',
               'Master group code is required, e.g. PARLE or CURRENT') as err
        from pg_temp._imp where app.imp_text(r ->> 'master_code') is null
      union all
      select app.imp_err(row_no, 'master_code', r ->> 'master_code',
               'No master group exists with this code — import the Master Groups sheet first')
        from pg_temp._imp i
       where app.imp_text(i.r ->> 'master_code') is not null
         and not exists (select 1 from public.master_group m
                          where m.code = upper(app.imp_text(i.r ->> 'master_code')))
    ) t;
  end if;
  ...
  if p_entity = 'master_group' then
    insert into public.master_group (code, name)
    select upper(app.imp_text(r ->> 'code')), app.imp_text(r ->> 'name')
      from pg_temp._imp where not is_existing;
    get diagnostics v_imported = row_count;

    if p_update_existing then
      update public.master_group t
    ...
    insert into public.product_group (code, name, master_group_id)
    select app.imp_text(i.r ->> 'code'), app.imp_text(i.r ->> 'name'), m.id
      from pg_temp._imp i
      join public.master_group m
        on m.code = upper(app.imp_text(i.r ->> 'master_code'))
...
comment on function public.import_masters(text, jsonb, boolean, boolean, boolean) is
  'Bulk import of master data from a spreadsheet: master groups, routes, product
   groups, suppliers, parties and products. A code that already exists is an
   error by default, skipped with p_skip_existing, or overwritten from the sheet
   with p_update_existing. Nothing is written unless every row is good.';
